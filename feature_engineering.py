...
def get_tokens(url):
    """Custom tokenizer for URLs, splitting by common delimiters and filtering non-informative tokens."""
    tokens = re.split(r'[/\-\.]', url)
    tokens = [t for t in tokens if t and t != 'com' and t != 'www']
    return list(set(tokens))

def extract_lexical_features(df):
    """Extracts lexical and structural features from URLs in a DataFrame."""
